...
-(void)addSubViewWithSubView:(NSArray *)imageArray andTitle:(NSArray *)titleArray andSize:(CGSize)size andcenterImage:(UIImage *)centerImage
{
    ...
    for(NSInteger i=0; i<_numOfSubView ;i++){
        ...
        [button setTitleColor:[UIColor colorWithRed:0.33 green:0.33 blue:0.33 alpha:1] forState:0];
        button.imageEdgeInsets  = UIEdgeInsetsMake(-20, 24, 0, 0);
        button.titleEdgeInsets = UIEdgeInsetsMake(100, 0, 20, 0);
    ...
    if(centerImage==nil){
        ...
        [buttonCenter addTarget:self action:@selector(btnTouched:) forControlEvents:UIControlEventTouchUpInside];
    }else{
...
- (void)btnTouched:(UIButton *)sender{
    [self.delegate didSelectedBtnIndex:sender.tag-100];
}
