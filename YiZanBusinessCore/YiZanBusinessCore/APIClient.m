...
/*
//#define ENC
static NSString* const  kAFAppDotNetAPIBaseURLString    = @"http://192.168.1.114:3000/staff/1.0/";
*/


static NSString* const  kAFAppDotNetAPIBaseURLString    = @"sq.test.o2o.fanwe.cn/staff/v1/";
//#define ENC
//static NSString* const  kAFAppDotNetAPIBaseURLString    = @"http://api.sqtest.vso2o.jikesoft.com/staff/v1/";
//NSString*  kAFAppDotNetAPIBaseURLString    = @"http://192.168.1.199/staff/v1/";
...
+ (instancetype)sharedClient {
    ...
    dispatch_once(&onceToken, ^{
        _sharedClient = [[APIClient alloc] initWithBaseURL:[NSURL URLWithString:[APIClient APiWithUrl:@"api" andOtherUrl:nil]]];
...
-(void)postUrl:(NSString *)URLString parameters:(id)parameters call:(void (^)(  SResBase* info))callback
{
    ...
    NSString* token = [APIClient getmToken];
    ...
    if( [SUser currentUser].mUserId)
    {
        [tparam setObject:[NSString stringWithFormat:@"%d",[SUser currentUser].mUserId] forKey:@"userId"];
    }
    
    ///这2个参数是带再外面的
    MLLog(@"链接：%@参数%@,%@",URLString,tparam,parameters);
...
+ (NSString *)APiWithUrl:(NSString *)mHeader andOtherUrl:(NSString *)mUrl{
    ...
    NSString *APIUrl = [NSString stringWithFormat:@"http://%@.%@%@",mHeader,kAFAppDotNetAPIBaseURLString,mUrl];
    return APIUrl;
    
}
+ (NSString *)wapWithUrl:(NSString *)path{
    return [APIClient APiWithUrl:@"wap" andOtherUrl:path];
}
